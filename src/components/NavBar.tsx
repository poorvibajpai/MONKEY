    ...
    <div className="relative w-full bg-black text-white">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        <div className="w-[15%]">
          ...
          <Link to={"/"}><img src={logo} alt="" className="w-full max-h-[40px] object-cover object-center" /></Link>
        </div>
          ...
          <ul className="inline-flex space-x-8">
            {navItems.map((item) => (
              <li key={item.name}>
                <NavLink
                  to={item.to}
                  className={`menu-links text-base font-semibold ${item.name == "Sale" ? "text-red-300 hover:text-red-400" : "text-white hover:text-white"} `}
